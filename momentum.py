...
class CrossAssetMomentum():
    def __init__(self, prices, lookback_period, holding_period, n_selection, cost=0.001, signal_method='dm', weightings='emv', long_only=False):   
        ...
        self.holding_returns = self.get_holding_returns(self.returns, holding_period)
        ...
        if weightings == 'ew':
            self.cr_risk_weight = self.equal_weight(self.signal)
        elif weightings == 'emv':
            self.cr_risk_weight = self.equal_marginal_volatility(self.returns, self.signal)

        self.rebalance_weight = 1 / holding_period
        self.cost = self.transaction_cost(self.signal, cost)

        self.backtest_res = self.backtest(self.holding_returns, self.signal, self.cost, self.rebalance_weight, self.cr_risk_weight)
    ...
    def get_holding_returns(self, returns, holding_period):
        ...
        """
        holding_returns = ((1 + returns).rolling(holding_period).apply(np.prod) - 1).shift(-holding_period).fillna(0)
